...
def snapdeal_detail(URL):
    snapdeal_product_type = {
        ...
        "furniture": ['pdp-auto-topcenter-inner layout', 'furniturePriceTile row', 'col-xs-22'],
    ...
    for i in snapdeal_product_type:
        try:
            frame_4_common = frame_3_common.find(
                'div', class_=snapdeal_product_type[i][0])  # pdp-elec-topcenter-inner layout # pdp-fash-topcenter-inner layout #pdp-auto-topcenter-inner layout
            frame_5_price = frame_4_common.find(
                'div', class_='container-fluid reset-padding')
            frame_6_price = frame_5_price.find(
                'div', class_=snapdeal_product_type[i][1])  # elecPriceTile buyNowBlock row # fashionPriceTile row #furniturePriceTile row
